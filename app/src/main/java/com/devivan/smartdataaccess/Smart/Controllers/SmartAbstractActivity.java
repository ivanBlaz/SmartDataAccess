...
public abstract class SmartAbstractActivity extends AppCompatActivity {
    ...
    protected void setSmartContentView(int layoutResID, int... smartViews) {
    ...
    protected void displayData(View view) {
        ...
        if ((isButton(view) || isImageView(view)) && key.split(SEPARATOR).length == 2) { // Button
            ...
            @SuppressLint("Recycle") TypedArray layouts = getResources().obtainTypedArray(R.array.smartLines);
            resource = layouts.getResourceId(Integer.parseInt(recyclerView.getTag().toString()), -1);
                    ...
                    if  (key.split(SEPARATOR).length == 2) {
                        ...
                        if (FILTER.equals(action)) {
                            ...
                            if (listRecyclerViews.stream().anyMatch(rv -> getKey(rv).endsWith(bean))) {
                                ...
                                @SuppressLint("Recycle") TypedArray layouts = getResources().obtainTypedArray(R.array.smartLines);
                                int resource = layouts.getResourceId(Integer.parseInt(recyclerView.getTag().toString()), -1);
